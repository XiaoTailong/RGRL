...
class QIsingEnv(gym.Env):
    ...
    def __init__(self, num_qubits=6, target_param=None, agent_param=None, r_dim=32,
        ...
        self.average_steps = 3  # or 7
        self.average_count = 0
        self.average_memory = np.zeros((self.average_steps, r_dim), dtype=np.float32)
    ...
    def reset(self, seed=None):
        ...
        # self.average_count = 0
        #
        # self.average_memory[self.average_count, :] = agent_observation
    ...
    def step(self, action):
        ...
        if self.average_count % self.average_steps == 0:
            # 算相邻步骤的平均表征，得到奖励信号
            average_observation = np.mean(self.average_memory, axis=0)
            normalized_euclidean_distance = (np.linalg.norm(average_observation - self.target_observation)
                                             / np.sqrt(average_observation.shape[0]))
        ...
        info = self._get_info(agent_state, self.target_state)
        if reward == 0 or info['param_distance'] < 1e-5:
            done = 1
        else:
            done = 0
        truncated = False
