...
class QIsingEnv(gym.Env):
    def __init__(self, num_qubits=6, target_param=None, agent_param=None,
                 maximum_counter=512, small_range=0.2,
    ...
    def step(self, action):
        ...
        action_step = self.action_step + (0.01 - self.action_step) * self.counter / self.maximum_counter

        # choice one
        # frac = 1.0 - (self.counter - 1.0) / self.maximum_counter
        # action_step = frac * self.action_step
        # choice two
        # if self.counter < self.maximum_counter / 2:
        #     action_step = self.action_step
        # else:
        #     action_step = self.action_step*0.5
