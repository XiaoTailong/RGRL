...
from QENV_small_range_nodone import QIsingEnv
...
if __name__ == "__main__":
    ...
    # 不同的seed，不同的目标参数
    run_name = f"{args.exp_name}_seed{args.seed}_target{args.target_case}_as{args.action_step}_nv{args.n_views}"
    ...
    if args.target_case == 1:
        ...
        target_param = np.array([0.2, -0.8, 0.2, -0.8, 0.2, -0.8])
    elif args.target_case == 3:
        target_param = np.array([0.2, 0.2, 0.2, -0.8, -0.8, -0.8])
    ...
    env = QIsingEnv(target_param=target_param, agent_param=agent_param,
        ...
        for step in range(0, args.num_steps):  # policy roleout 128
            ...
            if global_step % args.num_steps == 0 or global_step == 1:
                print(f"global_step={global_step}, quantum_fidelity={info['quantum_fidelity']},"
                      f" reward={reward}, param_dis={info['param_distance']}", flush=True)
