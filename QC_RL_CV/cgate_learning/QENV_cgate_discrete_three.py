...
class QCVENV(gym.Env):
    def __init__(self, target_param=None, initial_param=None,
        ...
        if target_param is None:
            ...
            self.target_param = np.array([np.random.random(), np.abs(np.random.random() * 3), np.random.random() * np.pi * 2])
        else:
        ...
        self.action_space = spaces.MultiDiscrete(np.array([3, 3, 3]))
    ...
    def reset(self, seed=None):
        ...
        agent_small_change = np.random.uniform(-0.01, 0.01, 3)
    ...
    def step(self, action):
        ...
        action_step_all = self.action_step * np.array([1, 3, 2*np.pi])
        ...
        for i, action_element in enumerate(action):
            ...
            if i == 0:
                self.agent_param[i] = np.clip(self.agent_param[i] + action_change[i], 0, 1)
            elif i == 1:
