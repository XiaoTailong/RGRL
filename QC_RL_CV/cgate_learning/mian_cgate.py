...
if __name__ == "__main__":
    ...
    # 不同的seed，不同的目标参数
    run_name = f'{args.exp_name}_seed{args.seed}_target_case{args.target_case}_ini{args.agent_random}_nv{args.n_views}_as{args.action_step}'
    ...
    target_param = np.array([np.random.random() * 1, np.abs(np.random.random() * 3),
                             np.random.random() * np.pi * 2])

    print("target_param:", target_param, flush=True)

    initial_param = np.array([np.random.random() * 1, np.abs(np.random.random() * 3),
                         np.random.random() * np.pi * 2])

    param_distance1 = np.linalg.norm(target_param - initial_param, ord=1)
    param_distance2 = np.linalg.norm(target_param + initial_param, ord=1)
    ...
    while param_distance < 1.5 and param_distance > 2.5:
        initial_param = np.random.random(2) * 4 - 2
    ...
    for update in range(1, num_updates + 1):
        ...
        if global_step % args.num_steps*50 == 0:
