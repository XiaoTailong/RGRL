...
from QENV_cgate_discrete import QCVENV
...
if __name__ == "__main__":
    ...
    # 不同的seed，不同的目标参数
    run_name = f'{args.exp_name}_seed{args.seed}_nv{args.n_views}_as{args.action_step}_90'
    ...
    target_param = np.array([np.abs(np.random.random() * 3),
                             np.random.random() * np.pi * 2])

    print("target_param:", target_param, flush=True)

    initial_param = np.array([np.abs(np.random.random() * 3),
                         np.random.random() * np.pi * 2])

    param_distance = np.linalg.norm(target_param - initial_param, ord=1)
    ...
    #     initial_param = np.random.random(3) * 4 - 2
    #
    #     param_distance = np.linalg.norm(target_param - initial_param, ord=1)
        # param_distance2 = np.linalg.norm(target_param + initial_param, ord=1)
        ...
        for step in range(0, args.num_steps):  # policy roleout 128
            ...
            if global_step % args.num_steps == 0 or global_step == 1:
                print(f"global_step={global_step}, quantum_fidelity={info['quantum_fidelity'][0]},"
                      f" reward={reward[0]}, param_dis={info['param_distance'][0]}, "
                      f" agent_param={info['agent_param'][0]}", flush=True)
