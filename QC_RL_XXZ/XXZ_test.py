...
import gym
# import pandas as pd
from XXZ_ENV_small_change_model1_rfixed_fixed_epi import XXZEnv
from main_small_change_model1_rfixed import Agent
...
run_name = f"{args.exp_name}_seed{args.seed}_target{args.target_case}_nv{args.n_views}_action_complete_{args.target_case}"

ids = np.arange(1, 64, 3)
if args.target_case == 1:
    # initial_agent_id = np.array([ids[8], ids[18]], dtype=np.int32)
    # target_param_id = np.array([46, ids[10]], dtype=np.int32)
    initial_agent_id = np.array([ids[8], ids[18]], dtype=np.int32)
    target_param_id = np.array([ids[15], ids[10]], dtype=np.int32)
elif args.target_case == 2:
    # target_param_id = np.array([7, 7], dtype=np.int32)
    # initial_agent_id = np.array([ids[7], ids[16]], dtype=np.int32)
    target_param_id = np.array([7, ids[9]], dtype=np.int32)
    initial_agent_id = np.array([ids[7], ids[16]], dtype=np.int32)
    ...
    # 会比较难学习
    target_param_id = np.array([ids[18], ids[5]], dtype=np.int32)
...
model_name = f"main_small_change_model1_rfixed_42_{args.target_case}_ns512_nv50_action_complete_case1_10"
...
observation_save = torch.zeros((num_repetitions, 512, 96)).to(device)
...
np.save('./exp/' + run_name + '_obs_case_10', observation_save.cpu().numpy())
np.save('./exp/' + run_name + '_action_case_10', actions_save.cpu().numpy())
np.save('./exp/' + run_name + '_reward_case_10', rewards_save.cpu().numpy())
np.save('./exp/' + run_name + '_fidelity_case_10', fidelity_save.cpu().numpy())
np.save('./exp/' + run_name + '_distance_case_10', distance_save.cpu().numpy())
