...
if __name__ == "__main__":
    ...
    if args.target_case == 1:
        ...
        target_param_id = np.array([52, ids[6]], dtype=np.int32)
    elif args.target_case == 2:
        target_param_id = np.array([7, ids[6]], dtype=np.int32)
        ...
        initial_agent_id = np.array([52, ids[6]], dtype=np.int32)
    elif args.target_case == 4:
        # 会比较难学习
        target_param_id = np.array([ids[5], ids[6]], dtype=np.int32)
        ...
        initial_agent_id = np.array([ids[1], ids[6]], dtype=np.int32)
        ...
        for step in range(0, args.num_steps):  # policy roleout 128
            ...
            if global_step % args.num_steps == 0 or global_step == 1:
                print(f"global_step={global_step}, quantum_fidelity={info['quantum_fidelity']},"
                      f" reward={reward}, param_J={info['param_J']}, param_delta={info['param_delta']}",
                      flush=True)
