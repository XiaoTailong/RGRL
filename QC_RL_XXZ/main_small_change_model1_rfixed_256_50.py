...
from XXZ_ENV_small_change_model1_rfixed_50 import XXZEnv
...
class Agent(nn.Module):
    def __init__(self, env):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(env.observation_space.shape[0], 128)),
            nn.Tanh(),
...
if __name__ == "__main__":
    ...
    # 不同的seed，不同的目标参数
    run_name = f"{args.exp_name}_{args.seed}_{args.target_case}_ns{args.num_steps}_nv{args.n_views}"
    ...
    if args.target_case == 1:
        ...
        # target_param_id = np.array([ids[18], ids[6]], dtype=np.int32)
        initial_agent_id = np.array([ids[8], ids[20]], dtype=np.int32)
        target_param_id = np.array([52, 10], dtype=np.int32)
    elif args.target_case == 2:
        # target_param_id = np.array([ids[5], ids[5]], dtype=np.int32)
        # initial_agent_id = np.array([ids[8], ids[17]], dtype=np.int32)
        ...
        # initial_agent_id = np.array([ids[18], ids[6]], dtype=np.int32)
        target_param_id = np.array([ids[8], ids[18]], dtype=np.int32)
        initial_agent_id = np.array([52, 10], dtype=np.int32)
    elif args.target_case == 4:
        target_param_id = np.array([ids[5], ids[5]], dtype=np.int32)
        initial_agent_id = np.array([ids[16], ids[16]], dtype=np.int32)
    elif args.target_case == 5:
        # target_param_id = np.array([ids[8], ids[17]], dtype=np.int32)
        # initial_agent_id = np.array([ids[5], ids[5]], dtype=np.int32)
        target_param_id = np.array([ids[8], ids[20]], dtype=np.int32)
        initial_agent_id = np.array([ids[2], ids[2]], dtype=np.int32)
    elif args.target_case == 6:
        # 会比较难学习
        # target_param_id = np.array([ids[18], ids[6]], dtype=np.int32)
        # initial_agent_id = np.array([ids[1], ids[20]], dtype=np.int32)
        target_param_id = np.array([ids[18], ids[4]], dtype=np.int32)
        initial_agent_id = np.array([ids[1], ids[20]], dtype=np.int32)
    else:
