...
class XXZEnv(gym.Env):
    def __init__(self, target_param_id=None, agent_param_id=None,
                 maximum_counter=50,
                 r_dim=16, h_dim=48, z_dim=16, L=12,
        ...
        try:
            ...
            self.model.load_state_dict(torch.load(
                current_directory + '/models/XXZ_50qubit_partial3_16_48_16_softmax_cpu', map_location='cpu'))
    ...
    def reset(self, seed=None):
        ...
        iJ = np.random.choice(ids)
        id = np.random.choice(ids)
        self.agent_param_id = np.array([iJ, id], dtype=np.int32)
    ...
    def step(self, action):
        ...
        reward = -1 * normalized_euclidean_distance * 100  
